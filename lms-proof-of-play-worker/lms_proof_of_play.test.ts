...
// ---------------------------------------------------------------------
// Init de la vraie pool hebdomadaire (2026-W37) avec l'autorité backend
// réelle du worker Cloudflare — pas un wallet de test.
// ⚠️ Supprime ce bloc (et la clé ci-dessous) juste après l'avoir lancé.
// ---------------------------------------------------------------------

describe("init pool réelle", () => {
  const provider2 = anchor.AnchorProvider.env();
  const program2 = anchor.workspace.LmsProofOfPlay as Program;

  // Colle ta clé base58 (celle de SOLANA_AUTHORITY_SECRET_KEY) ici
  // UNIQUEMENT pour ce test. Ne commit jamais ce fichier avec la clé dedans.
  const authoritySecret = "COLLE_TA_CLE_BASE58_ICI";
  // bs58 n'est pas installable dans Solana Playground -> on utilise
  // l'implémentation déjà embarquée par Anchor, comportement identique.
  const authority = Keypair.fromSecretKey(
    anchor.utils.bytes.bs58.decode(authoritySecret)
  );

  const weekId2 = "2026-W37"; // le weekId réel que ton worker va tester

  const [weeklyPoolPda2] = PublicKey.findProgramAddressSync(
    [Buffer.from("pool"), Buffer.from(weekId2)],
    program2.programId
  );
  const [vaultPda2] = PublicKey.findProgramAddressSync(
    [Buffer.from("vault"), Buffer.from(weekId2)],
    program2.programId
  );

  it("vérifie/airdrop l'autorité", async () => {
    const bal = await provider2.connection.getBalance(authority.publicKey);
    console.log("solde authority:", bal / anchor.web3.LAMPORTS_PER_SOL, "SOL");
    if (bal < 0.1 * anchor.web3.LAMPORTS_PER_SOL) {
      const sig = await provider2.connection.requestAirdrop(
        authority.publicKey,
        0.5 * anchor.web3.LAMPORTS_PER_SOL
      );
      await provider2.connection.confirmTransaction(sig);
    }
  });

  it("crée un mint de test SKR", async () => {
    const payer2 = (provider2.wallet as any).payer;
    const mintReal = await createMint(provider2.connection, payer2, authority.publicKey, null, 6);
    console.log("MINT créé :", mintReal.toBase58()); // note-le, tu en auras besoin
  });

  it("initialise la pool réelle pour", async () => {
    // remplace MINT_ICI par le mint loggé juste au-dessus
    const mint2 = new PublicKey("64TQTAykeYQmRmeqMDKB2kWJGGSp8FuGLpEkcJfdwSY6");
    await program2.methods
      .initializeWeeklyPool(weekId2)
      .accounts({
        authority: authority.publicKey,
        weeklyPool: weeklyPoolPda2,
        mint: mint2,
        vault: vaultPda2,
        tokenProgram: TOKEN_PROGRAM_ID,
        systemProgram: SystemProgram.programId,
      ...
      .signers([authority])
      .rpc({ commitment: "confirmed" });
    console.log("Pool initialisée pour", weekId2);
  });
});
